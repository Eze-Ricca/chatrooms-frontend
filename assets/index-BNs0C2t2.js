...
`)+R+`return __p
 ...
 <main class="main">
  ...
  `;const s=We.getState().realtimeDBRoomId;function a(l){We.pushMessage(l),console.log("Mensaje enviado correctamente!")}return(()=>{const l=pd(GI,`/rooms/${s}/messages`);_d(l,_=>{const g=_.val()||{},b=i.querySelector(".container-messages");b.innerHTML="";function E(){b.scrollTop=b.scrollHeight}Object.values(g).forEach(C=>{const A=document.createElement("li"),H=new Date(C.timestamp).toLocaleTimeString("es-AR",{hour:"2-digit",minute:"2-digit"}),G=C.author===We.data.nombre;A.classList.add(G?"usuario-message":"otro-message"),A.innerHTML=`
        ...
        `,b.appendChild(A),E()})});const u=i.querySelector(".input-mesagge"),f=i.querySelector(".button");f==null||f.addEventListener("click",()=>{const _=u.getValue();_.trim()!==""&&(a(_),u.setValue(""))}),u.addEventListener("keypress",_=>{if(_.key==="Enter"){_.preventDefault();const g=u.getValue();g.trim()!==""&&(a(g),u.setValue(""))}})})(),i.appendChild(e),i}const $i=document.querySelector("#app");if(!$i)throw new Error("Elemento #app no encontrado");const KI=location.hostname==="localhost"||location.hostname==="127.0.0.1"||location.hostname.startsWith("192.168.")||location.hostname==="[::1]";KI||Zt.base("/chatrooms-frontend");function YI(i){Zt(i)}Zt("/",()=>Zt.redirect("/home"));Zt("/home",()=>{$i.innerHTML="",$i.appendChild(HI({goTo:YI}))});Zt("/chat",()=>{$i.innerHTML="",$i.appendChild(VI())});function QI(){Zt.start()}QI(),We.init();
